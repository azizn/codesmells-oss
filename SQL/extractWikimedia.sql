CREATE DEFINER =`apatta`@`%`PROCEDURE `extractWikimedia`(IN var1 varchar(100))
DETERMINISTIC
BEGIN
...
if v_not_found then
 ...
 SET@sql_text2 = CONCAT('INSERT INTO tempWikimedia      SELECT id,''',
var1,''',message,''OpenStack'',''',temp_synonyms,''',
created_on,
reviewer,author FROM comment_detail_Wikimedia where message like 
',@sql_text1,';');
